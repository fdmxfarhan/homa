...
class Map:
    ...
    #         for j in range(self.width):
    #             if(self.isBomb(i, j)):
    #                 path=self.findPath(me.getPos(), {'x': i, 'y': j})
    #                 if(path!=None):
    #                     dist = len(list(path))
    ...
    def findBestPlaceForBomb(self, box):
        ...
        for i in range(1, self.bombRange+1):
            ...
            elif(self.isWalkable(box['x']+i, box['y'])):
                destinations.append({'x': box['x']+i, 'y': box['y']})
            elif(self.isWalkable(box['x'], box['y']+i)):
    ...
    def findPath(self, source, target):
        ...
        while len(open_list) > 0 and cnt < (width*height)/4:
            cnt += 1
    ...
    def findBestDirectionForTrap(self, me, enemy):
        if(me.y < enemy.y):
            if(not self.isTrap(me.x, me.y+1) and self.findPath(enemy.getPos(), {'x': me.x, 'y': me.y+1})):
                return 'right'
            return False
        else:
            if(not self.isTrap(me.x, me.y-1) and self.findPath(enemy.getPos(), {'x': me.x, 'y': me.y+1})):
...
def AI():
    ...
    if(stepCount > deadZoneStartingStep-len(pathToCenter)*2):
        # log(len(pathToCenter))
        me.destination = centerPos
        me.target = 'deadZone'
    elif(enemyClose and me.trapCount > 0):
